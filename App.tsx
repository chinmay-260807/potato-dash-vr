...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20">
      {/* Header Section */}
      <header className="potato-gradient pt-12 pb-24 px-6 text-white text-center relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          <h1 className="font-heading text-6xl mb-4 drop-shadow-lg tracking-wider animate-bounce">
            POTATO DASH VR
          </h1>
      ...
      {/* Navigation */}
      <nav className="max-w-6xl mx-auto -mt-8 px-4 flex justify-center">
        <div className="bg-white rounded-2xl shadow-xl p-2 flex gap-2 w-full max-w-3xl border border-amber-100 overflow-x-auto">
          {(['concept', 'tech', 'physics', 'uiux', 'roadmap'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 min-w-[100px] py-3 px-4 rounded-xl font-bold transition-all whitespace-nowrap ${
                activeTab === tab 
      ...
      <main className="max-w-6xl mx-auto px-6 mt-12">
        {activeTab === 'concept' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-6">
                  <div className="bg-amber-100 p-4 rounded-xl border-l-4 border-amber-500">
            ...
            {/* Obstacles */}
            <section>
              <h2 className="font-heading text-4xl text-amber-900 mb-8 text-center">Kitchen Hazards</h2>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-amber-100 py-4 px-6">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="text-2xl">🥔</span>
            <span className="font-heading text-amber-900">POTATO DASH VR</span>
          </div>
          <p className="text-xs text-gray-400 font-medium">© 2024 Spud Studios • Internal Design Blueprint</p>
          <div className="flex gap-4">
            <button className="text-xs font-bold text-amber-600 hover:underline">Download PDF</button>
            <button className="text-xs font-bold text-amber-600 hover:underline">Share Blueprint</button>
          </div>
        </div>
      </footer>
    </div>
